...
begin
  ...
  if length(trim(coalesce(new.codinep, ''))) > 0 then
    select into v_registros coalesce((select sotech.esus_inep.pkinep from sotech.esus_inep where sotech.esus_inep.codinep = new.codinep and case when tg_op = 'UPDATE' then sotech.esus_inep.pkinep <> new.pkinep else 1 = 1 end order by 1 limit 1), -1) as codigo;
    if v_registros.codigo <> -1 then
      v_erro := sotech.sys_set_erro(v_erro, 'Registro já cadastrado! Tipo parto: ' || new.codinep || ' -> Cód:' || v_registros.codigo::text);
    end if;
  end if;
  -- final
  v_registros := null;
